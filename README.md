...
**2** **、重载后置** **++** **、** **--** **时函数中需要一个** **int** **类形参（哑元），形参在函数体中不发挥作用，只用来区分不同的重载形式。**

例：Time operator++(int);

前置++、--和后置++、--的区别，给还不懂的童鞋解释一下：
...
**3.关于前置自增运算符重载和后置自增运算符重载中的“&”有无的辨析**（以时间类为例）
...
前置自增比后置自增多了一个“&”（引用），其原因与其含义有关。

一般情况下可以认为前置自增是先加一再参与运算，后置自增是先参与运算再加一，但其实后置自增在参与运算的时候已经加一，只是参与运算的是没有加一之前的替代值。所以前置自增运算符的重载需要使用&（引用）,此处引用是本类对象的引用，因为前置自增的结果是加一，然后使用，使用的就是函数返回的值（*this），而返回的又是他自己，所以需要一个引用。

后置自增中old可以看作是*this的副本，使用的是没有加一的值（old），但是使用之前自身（*this）已经加一，后置自增使用不到自身，后置自增生成临时变量，临时变量的生存期很短，用完即被释放，不需要返回引用类型
...
Clock Clock::operator++(int){//后置++运算符
//注意形参表中的整型参数
 Clock old=*this;//生成副本
 ++(*this);//调用前置“++”运算符，原对象在此时已经“悄悄”加1
 return old;//返回的是副本形式，不是原对象
}
```

#### **2.2 为什么后置++运算符的代码里调用了前置++运算符？**

便于在修改前置++运算符的代码之后，后置++运算符也能保证功能一致。

#### **2.3 为什么return old?**

说明*this已经++，但返回的是没有++的old，这也解释了前置++和后置++的区别。
...
哑元  :  dummy表示虚无的元素，没有实际空间，甚至连名字都可以没有，它只有联系上实元才有意义。C函数中明显具有哑元特性的应该是可变参数表，如int  printf(   const char  *format [,   argument]...  );在C++的运算符重载中，就会用到哑元以区分i++与++i的区别
...
​     如果重载的函数，其解决问题的逻辑是一致的、函数体语句相同，只是处理的数据类型不同，那么写多个相同的函数体，是重复劳动，而且还可能因为代码的冗余造成不一致性。这时可以使用函数模板。
...
其中，template是一个声明模板的关键字，它表示声明一个模板。 template<class T1,class T2,… >为模板函数的声明。 class T1,class T2,…为模板形参——T1、T2为虚拟类型参数。可把模板形参改为：typename T1,typename T2,…
...
　　一个类模板(类生成类)允许用户为类定义一种模式，使得类中的某些数据成员、默认成员函数的参数，某些成员函数的返回值，能够取任意类型(包括系统预定义的和用户自定义的)。只要为一批类所组成的整个类家族创建一个类模板，给出一套程序代码，就可以用来生成多种具体的类，（这类可以看作是类模板的实例），从而大大提高编程的效率。
...
类名，在类定义体中，如果采用通用数据类型的成员，函数参数的前面需加上T，其中通用类型T可以作为普通成员变量的类型。还可以作为成员函数的参数和返回类型等。
...
模板类是类模板实例化后的一个产物，说个具体点的例子吧，我们把类模板比作是一个做饼干的模子，而模板类就是用这个模子做出来的饼干，至于饼干具体是什么味道，就要看实例化时用的材料了，可以是巧克力饼干，也可以是牛奶饼干，这些饼干除了材料不一样外，其它的东西都是一样的。
...
1.数组的最后一个元素是'\0', '\0'是判定字符数组结束的标识，表示这串字符到结尾了，它是占位置的！

2.数组名本质是指针常量（地址），所以声明函数参数的时候二维数组可以用二维指针来声明。例：**p

3.在输出字符数组的时候 '\o'不会被输出，但是统计输出字符的数量的时候，'\o'是会作为一个字符被统计的。

4.当输入输出运算符重载为模板类的友元函数时，如果只是在类体中写为ostream& operator<<(ostream& out,MyClass<T>& c),编译时将会出现连接错误。解决方法：改变友元与模板类的对应关系为多对多。例如：
...
使用模板时会遇到一些特殊的类型需要特殊处理，不能直接使用当前的模板函数，所以此时我们就需要对该类型特化出一个模板函数（就是写出一个模板函数专门给该类型使用），例如比较两参数是否相同时，字符串参数无法直接用==比较，可以使用函数的特化解决。下面是例子：
...
例：ptr->getx()相当于(*ptr).gex();

 **指向对象数据成员的指针**

**定义指向对象的数据成员的指针变量的方法和定义指向普通变量的方法类似**

**一般形式：数据类型名\*指针变量名；如：**

```c++
p=&date.day;//将对象date的数据成员day的地址赋给p1**

cout<<\*p1<<endl;//输出date.day的值
```
...
| 存储区         | 内容                       | 权限     |
| -------------- | -------------------------- | -------- |
| 栈区（stack）  | 函数的参数值、局部变量值   | 可读可写 |
| 堆区（heap）   | 动态申请的内存             | 可读可写 |
| 全局区或静态区 | 全局变量、static修饰的变量 | 可读可写 |
| 字符常量区     | 常量字符串                 | 只读     |
| 程序代码区     | 存放函数体的二进制代码     | 只读     |

 

注：一般情况下，从栈区到代码区，是从高地址到低地址。栈向下增长，堆向上增长。

**C++ ** **内存分配3种方式：**

- [1]从静态存储区域分配。内存在程序编译的时候就已经分配好，这块内存在程序的整个运行期间都存在。例如全局变量，static变量。
- [2]在栈上创建。在执行函数时，函数内局部变量的存储单元都可以在栈上创建，函数执行结束时这些存储单元自动被释放。
- [3]从堆上分配，亦称动态内存分配。程序在运行的时候用malloc或new申请内存，程序员自己负责在何时用free或delete释放内存。如果不调用delete或者free，则这块空间必须到软件运行结束后才能被系统回收。

注：栈由系统自动分配速度较快，堆由new分配速度较慢，且容易产生内存碎片，但使用方便。

NEW:动态申请内存操作符

功能：在程序执行期间，申请用于存放内存的T类型对象的存储空间，并依初值列表赋予初值。

结果:若成功，为T类型的指针，只相信分配的内存；若失败，抛出异常。

### **三、析构函数**

**1.什么是析构函数**

析构函数是**类的一个成员函数**，名字由**位取反符~**接**类名**构成，没有返回值，不接受参数，一个类只有一个析构函数。当一个类未定义自己

的析构函数时，编译器会为它定义一个合成的析构函数，因此许多类不需要显式地编写析构函数。
...
- 1）在析构函数中，首先执行函数体，然后按照初始化顺序的逆序销毁成员。

- 2）对于动态分配的对象，当对指向它的指针使用delete运算符时被执行。

4.**执行时间**

析构函数在函数生存期结束时执行，不重载，无参数。
...
Time(int xx=0,int yy=0){x=xx;y=yy};
...
delete：删除动态区域，执行析构函数（同一个空间被delete两次会发生错误）

ps：delete需要注意加上[ ]，否则可能会造成运行时错误

###  **五、浅复制、深复制、浅赋值、深赋值**

**1、浅拷贝**

  两个对象通过默认的拷贝构造，指向了同一空间；在最后调用析构函数时，形成了对同一空间的多次释放，是错误操作！！！

**2、深拷贝**

   真实开辟空间拷贝数据的拷贝构造称为深拷贝。

**3、浅赋值**

​    两个对象通过默认的赋值语句，指向了同一空间。

**4、深赋值**

​      对象给对象赋值时候调动赋值语句，会开辟新的空间，注意开辟空间要释放原有空间
...
优点：更加简洁便于管理；可在访问数组元素前检查下标是否越界

分配和释放动态数组：

分配：new 类型名T[数组长度]，数组长度可为任意表达式，在运行时计算。

释放：delete[]数组名p。 p必须时new分配得到的数组首地址。

6.Tips

- 1.在重载=运算符时，应先判断是否是自身，例：
...
- 2.可以创建一个临时指针，方便给动态数组加/减数字时使用，例：

```c++
template <class T>
int DynamicArray<T>::resize(unsigned int newSize, const T &z) {
  	int i; cout<<"\nresize is called";
	if (mallocSize == newSize) 
        return 0;
	if (mallocSize < newSize)
    {
		T *temp = array; array = new T[newSize];
		int mallo = mallocSize;
		mallocSize = newSize;
		for (i=0; i<mallo; i++) array[i] = temp[i];
		for (i=mallo; i<newSize; i++) array[i] = z;
		delete[] temp;
		return 1;
	}
    else
  	{
		...
		T *temp = array;
		array = new T[newSize];
		for(i=0; i<newSize; i++) array[i] = temp[i];
		delete[] temp;
		return -1;
	}
} 
